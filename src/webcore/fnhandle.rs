...
/// A type-erased version of [`FnHandle`], [`FnMutHandle`], or [`FnOnceHandle`].
/// 
/// This has all the same functionality as [`FnHandle`], [`FnMutHandle`], and [`FnOnceHandle`]
/// but without having type parameters that encode the arguments and return value.
/// 
...
/// 
/// If you have a [`FnOnce`](::std::ops::FnOnce) function or closure that you want to call from JavaScript,
/// you can use it in the `js!` macro directly by using the [`Once`] wrapper, like this:
/// 
/// ```rust
...
/// ```
/// 
/// However, if you do this, you must remember to make sure that the callback is either called or dropped,
...
/// [`From`](::std::convert::From) trait, like this: `FnOnceHandle::from(f)`,
/// or or using [`Into`](::std::convert::Into), like this: `f.into()`
/// (type annotations may be needed for the latter).
/// 
/// # Example
/// 
/// ```rust
...
/// 
/// If you have a [`FnMut`](::std::ops::FnMut) function or closure that you want to call from JavaScript,
/// you can use it in the `js!` macro directly by using the [`Mut`] wrapper, like this:
/// 
/// ```rust
/// let callback = || { println!("Hello world!"); };
/// js! {
...
///     cb();
///     cb();
///     // Need to make sure to call drop() on cb, or else there is a memory leak.
///     cb.drop();
/// }
/// ```
/// 
/// However, if you do this, you must remember to drop the callback in JavaScrip manually,
/// otherwise the rust closure is never dropped and you have a memory leak.
/// 
...
/// [`From`](::std::convert::From) trait, like this: `FnMutHandle::from(f)`,
/// or or using [`Into`](::std::convert::Into), like this: `f.into()`
/// (type annotations may be needed for the latter).
/// 
/// # Example
...
/// 
/// If you have a [`Fn`](::std::ops::Fn) function or closure that you want to call from JavaScript,
/// you can use it in the `js!` macro directly, like this:
/// 
/// ```rust
/// let callback = || { println!("Hello world!"); };
/// js! {
...
/// [`From`](::std::convert::From) trait, like this: `FnHandle::from(f)`,
/// or or using [`Into`](::std::convert::Into), like this: `f.into()`
/// (type annotations may be needed for the latter).
/// 
/// # Example
...
impl GenericFnHandle {
	/// Create a new [`GenericFnHandle`] from a [`Reference`].
	/// 
	/// This should almost never be used in practice. Instead, a closure should be
	/// converted into a [`FnHandle`], [`FnOnceHandle`] or [`FnMutHandle`] using the
	/// rust standard library's [`From`] or [`Into`] methods, and those handles can (if necessary)
	/// be converted into [`GenericFnHandle`], again using [`From`] or [`Into`].
	/// A one-step conversion is not possible.
	...
	/// This method returns a [`Reference`] to the underlying JavaScript function.
...
impl< Args, Output > FnOnceHandle< Args, Output > {
	/// Leak the handle.
	/// 
	/// This means that the rust closure won't be dropped unless you call it or `.drop()` it from the JavaScript side.
	/// This method returns a [`Reference`] to the underlying JavaScript function.
	pub fn leak( self ) -> Reference {
        self.discarder.leak().0
    }
